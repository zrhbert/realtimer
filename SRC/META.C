...

{
  ...
  vqt_fontinfo (vdi_handle, &minADE, &maxADE, distances, &width, effects);
...

{
  ...
  else
  {
    ...
    Fseek (0L, f, 0); /* Nur Kopf einlesen */
    max_buffer = Fread (f, (LONG)sizeof (META_HEADER), meta_inf->buffer);
    ...
    file_split (meta_inf->name, &meta_inf->drive, meta_inf->path, NULL, NULL);
...

{
  ...
  {
    meta_index = 0;
    if (Fread (meta_f, 2L * meta_buflen, meta_buffer) == 0) return (-1);
  } /* if */
...

{
  ...
  if ((meta_inf->fsize > meta_inf->buflen) || (meta_inf->first))
  {
    ...
    Fseek (0L, meta_f, 0);
    meta_buflen = Fread (meta_f, 2L * meta_buflen, meta_buffer) / 2;
  } /* if */

  command = 0;
  Dsetdrv (meta_inf->drive);
...

{
  ...
  if (mode & SIZED)
  {
    ...
    if (meta_inf->best_fit)
      if ((diff.w < 0) || (diff.h < 0)) set_redraw (window, &window->scroll);
  } /* if */
} /* wi_snap */
...

{
  ...
  menu_icheck (popup, MFULL, ! meta_inf->best_fit);
  menu_icheck (popup, MBESTFIT, meta_inf->best_fit);

  item = popup_menu (popup, METAINFO, 0, 0, MINF, TRUE, mk->momask);
