...

{
  ...
  if (src_window->class == class_desk)          /* Objekte von Desktop */
  {
    ...
    switch (src_obj)
    {
      ...
      case IDISK    : action = DRAG_NOACTN;             break;
      case IPRINTER : action = DRAG_NOACTN;             break;
      case ICLIPBRD : icons_clipbrd (ICLIPBRD, ITRASH); break;
    } /* switch */
  } /* if */

  if (src_window->class == CLASS_CLIPBRD) action = DRAG_OK;
