...
/*****************************************************************************
- WI_JUNK Bug in hndl_mesag beseitigt, 04.01.95
29.11.94
- MILLISECS auf 10 runtergesetzt, von 100
18.04.94
- Kbshift nach evnt_multi ausgebaut, wg. neuem MidiShare
26.07.93
- WI_JUNK eingebaut
- cycle_windows auf CTRL-W umgebaut
20.06.93
- Kbshift nach evnt_multi eingebaut, wg. MidiShare Fehler
*****************************************************************************/
  ...
  if (top != NULL)
    if (top->flags & WI_MODAL)
      if ((mk->scan_code == F1) || (mk->scan_code == HELP))
      {
        ...
        if ((top->exit_obj != NIL) && ! is_state (top->object, top->exit_obj, DISABLED))
        {
...

{
  ...
  switch (msgbuff [0])                  /* Art der Nachricht */
  {
    ...
    case WM_REDRAW   :
		if (window) window->flags |= WI_JUNK;
...

{
  ...
  do
  {
    ...
    events  |= MU_M1;
    old_top  = new_top;
    top      = find_top ();             /* Hole oberstes Fenster von diesem Prozež */
    ...
    if (old_top != new_top)             /* Mausform durch Schliežen oder... */
