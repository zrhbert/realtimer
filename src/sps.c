...
WORD   icon;
{
  ...
  if (window != NULL)
  {
    ...
    window->showinfo  = info_mod;
      ...
      sprintf (window->name, "%s", sps_text [FSPSN].ob_spec.free_string);
...
WORD    icon;
{
	...
	if (window == NULL)
	{
		 ...
		 window = crt_dialog (sps_info, NULL, ISPS, sps_text [FSPSN].ob_spec.free_string, WI_MODAL);
	} /* if */
...
PRIVATE	RTMCLASSP create ()
{
	...
	daktstatus("Initialisierung SPS", "Start");
	...
	daktstatus("Initialisierung SPS", "Ende");
