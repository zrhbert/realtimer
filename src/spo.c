...
- Fehler in apply für y und z beseitigt
V 1.02
- import modernisiert
...
typedef struct spo_single
{
	...
	UINT	prop_x	: 1,	/* Flags für proportional an/aus */
			prop_y	: 1,
			prop_z	: 1;
} SPO_SINGLE;				/* Enthält alle SPO-Parameter eines einzelne Signals */

typedef struct setup
{
	SPO_SINGLE	spo_single [MAXSIGNALS];	/* Enthält die SPO-Informationen für die einzelnen Kanäle */
} SETUP;				/* Enthält alle Parameter einer kompletten SPO-Einstellung */
...
PRIVATE WORD	spo_rsc_hdr;					/* Zeigerstruktur für RSC-Datei */
PRIVATE WORD	*spo_rsc_ptr = &spo_rsc_hdr;		/* Zeigerstruktur für RSC-Datei */

PRIVATE WORD		instance_count = 0;			/* Anzahl der Instanzen */
PRIVATE CONST WORD max_instances = 20;			/* Max Anzahl Instanzen */
PRIVATE CONST STRING module_name = "SPO";		/* Name, für Extension etc. */
...
PUBLIC PUF_INF *apply	(RTMCLASSP module, PUF_INF *event)
{
	...
	REG WORD		k;		/* Temp. Variable wg. Überlauf */
	POINT_3D		*point;
