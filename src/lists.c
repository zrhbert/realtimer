...
GLOBAL VOID list_remove (LIST_P element)
{
  ...
  element->next = (LIST_P)NULL;
  element->prev = (LIST_P)NULL;
} /* list_remove */
...
GLOBAL LIST_P list_search_cmp (LIST_P header, KEYTYPE key, COMPARE_F compare)
{
    ...
    if (header) {
        element = header->next;
        while (element != header && compare (element->key,  key) !=0) {
...
GLOBAL LIST_P list_concat (LIST_P header1, LIST_P header2)
{
  ...
  /* copy first list */
  element = header1->next;
  while (element != header1) {
    list_rear_insert (header, list_new_el (element->key));
    element = element->next;
  } /* while */

  /* copy second list */
  element = header2->next;
  while (element != header2) {
    list_rear_insert (header, list_new_el (element->key));
    element = element->next;
  } /* while */
  } /* if header */
...
GLOBAL LIST_P list_new_el (KEYTYPE key)
{
	...
	if (key == NULL) return (LIST_P)NULL;
