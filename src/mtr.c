...
PUBLIC PUF_INF *apply	(RTMCLASSP module, PUF_INF *event)
{
	...
	LONG 				temp_x = 0L, temp_y=0L;	/* Hilfsvariable um öberlauf zu vermeiden */
...
WORD   icon;
{
  ...
  if (window != NULL)
  {
    ...
    window->showinfo  = info_mod;
      ...
      sprintf (window->name, "%s", mtr_text [FMTRN].ob_spec.free_string);
    sprintf (window->info, mtr_text [FMTRI].ob_spec.free_string, 0);
  } /* if */
...
WORD    icon;
{
	...
	if (window == NULL)
	{
		 ...
		 window = crt_dialog (mtr_info, NULL, IMTR, mtr_text [FMTRN].ob_spec.free_string, WI_MODAL);
	} /* if */
