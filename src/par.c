...
WORD   icon;
{
	...
	if (window != NULL)
	{
        ...
        sprintf (window->name, "%s", par_text [FPARN].ob_spec.free_string);
		/* sprintf (window->info, freetext [FPARI].ob_spec.free_string, 0); */
...
WORD    icon;
{
	...
	if (window == NULL)
	{
		 ...
		 window = crt_dialog (par_info, NULL, IPAR, par_text [FPARN].ob_spec.free_string, WI_MODAL);
	} /* if */
