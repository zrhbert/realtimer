...
/*****************************************************************************
V 1.00
- Var-Darstellung auf Bar geändert, 20.02.95
- MILLI auf 100 geändert, 09.02.95
- SysVAR eingebaut, 03.02.95
- panpan eingebaut, 04.01.95
V 0.14
- menu_title auf MCONTROLS geändert, 27.11.94
V 0.13 19.05.94
...
#define INITH  (13 * gl_hbox)           /* Anfangshöhe in Pixel */
...
typedef struct koo_signal_flags
{
	...
	UINT	voruz	: 1	;	/* Vor-und-Zurück-Wert anzeigen  */
...
} SFLAGS;				/* Enthält alle KOO-Parameter eines einzelnen Signals */
...
typedef struct koo_signal_val
{
	...
	WORD	voruz			;	/* Vor-und-Zurück-Wert */
	WORD	dehn			;	/* Dehnung */
	WORD	mtrspd		;	/* MTR-Geschw */
	WORD	track			;	/* CMI Signalzuweisung   */
	WORD  bigvol		;	/* Volume aus BIG-Sequenzer */
} SVALUES;				/* Enthält alle aktuellen Parameter eines einzelnen Signals */

typedef struct setup
{
	VOID *dummy;
} SETUP;		/* Enthält alle Parameter einer kompletten KOO-Einstellung */
...
typedef struct status
{
	...
	BOOLEAN	new;		/* Flag für komplett neuen Aufbau */
	LONG		var_values[MAXSETVARS];	/* Systemvariablen */
} STATUS;

/****** VARIABLES ************************************************************/
/* Resource */
PRIVATE WORD	koo_rsc_hdr;					/* Zeigerstruktur für RSC-Datei */
PRIVATE WORD	*koo_rsc_ptr = &koo_rsc_hdr;		/* Zeigerstruktur für RSC-Datei */
...
PRIVATE CONST STRING module_name = "KOO";		/* Name, für Extension etc. */
	...
	switch(type)
	{
		case SET_VAR:				/* Systemvariable auf neuen Wert setzen */
			if (variable == VAR_SET_KOO)
				...
				/* MTR-Beschleunigung für diesen Kanal setzen */
			...
			/* nicht nötig, wird über var_get_value abgefragt 
