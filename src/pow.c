...
} SETUP;	/* Enthält alle Parameter einer kompletten POW-Einstellung */
...
/****** VARIABLES ************************************************************/
PRIVATE WORD	pow_rsc_hdr;					/* Zeigerstruktur für RSC-Datei */
PRIVATE WORD	*pow_rsc_ptr = &pow_rsc_hdr;		/* Zeigerstruktur für RSC-Datei */
...
PRIVATE CONST STRING module_name = "POW";		/* Name, für Extension etc. */
...
PRIVATE BOOL		get_glove_data	(RTMCLASSP module)
{
	...
	while (Bconstat(port))
	{
		...
		/* Success ist TRUE wenn überhaupt eine Abfrage
			funktioniert hat. */
		success |= read_ok;
	} /* while */
...
PUBLIC VOID		precalc	(RTMCLASSP module)
{
	...
	/* Übertragung initialisieren wenn nötig */
	if (!status->connected && ! status->connect_aborted)
	{
